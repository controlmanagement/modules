...
class antenna_nanten(core.controller.antenna):
	...
	def galactic_move(self, l, b, off_x = 0, off_y = 0, hosei = 'hosei_230.txt', offcoord = "HORIZONTAL"):
		...
		press = float(condition[12])
		humid = float(condition[9])/100.
		self.antenna.thread_start('GLACTIC', 0, gx, gy, 0, 0, 0, temp, press, humid, 2600, 0, hosei, offcoord, off_x, off_y)
		return
	...
	def planet_move(self, number, off_x = 0, off_y = 0, hosei = 'hosei_230.txt', offcoord = "HORIZONTAL"):
		...
		press = float(condition[12])
		humid = float(condition[9])/100.
		self.antenna.thread_start('PLANET', number, 0, 0, 0, 0, 0, temp, press, humid, 2600, 0, hosei, offcoord, off_x, off_y)
		return
